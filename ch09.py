"""
'r' 〜 打开已有文件读取（默认），   若文件不存在会报错 | 文件夹不存在会报错

'x' 〜 创建一个新文件，打开，写操作，若文件已存在会报错 | 文件夹不存在会报错
'w' 〜 打开已有文件去写，会抹去原有内容，没有文件会新建 | 文件夹不存在会报错
'a' 〜 打开已有文件去写，在原内容后面写，没有文件会新建 | 文件夹不存在会报错
...
"""
# os.path.exsits 目录存不存在； os.makedirs 新建目录
