# python网站：https://www.python.org                   (chapter 1)
# pycharm网站：https://www.jetbrains.com/pycharm/      (chapter 1)
# VSCode网站：https://code.visualstudio.com            (chapter 1)
# python package index网站：https://pypi.org           (chapter 8)
# https://docs.python.org/3/library/logging.html#logrecord-attributes  (chapter 13)
# 正则测试网站：https://regexone.com | regex101.com     (chapter 16)
...
# python3.9 -m venv xxx  命令行创建虚似环境，其中的venv是个模块，是必须的。xxx是总文件夹名字。
# python3.9 -i xx.py     命令行进入交互式执行模式。

# 提示：python3.9 -m 的作用是在终端或命令行里，以脚本的形式运行模块 run library module as a script
# python3.9 -m pip install --upgrade setuptools (没有就安装，有就升级）
# python3.9 -m pip install wheel （相当于: pip install wheel）
# python3.9 setup.py sdist bdst_wheel  （基于setuptools模块和wheel模块，打包setup.py所描述的模块）
# python3.9 -m twine upload --repository-url https://upload.pypi.org.legacy/ dist/*
# twine upload --repository-url https://upload.pypi.org.legacy/ dist/*       (同上：发布模块)
